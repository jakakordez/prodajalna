    ...
    pesmiIzKosarice(zahteva, function(pesmi) {
      if (!pesmi) {
        ...
        for(var i = 0; i < stranke.length; i++){
          if(stranke[i].CustomerId == zahteva.session.narocnik){
            odgovor.setHeader('content-type', 'text/xml');
            odgovor.render('eslog', {
...
streznik.post('/prijava', function(zahteva, odgovor) {
  var form = new formidable.IncomingForm();
  zahteva.session.kosarica = false;
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    var napaka2 = false;
    try {
      var stmt = pb.prepare("\
    ...
    vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        odgovor.render('prijava', {sporocilo: napaka2?'Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.':'Stranka je bila uspešno registrirana.', seznamStrank: stranke, seznamRacunov: racuni});  
        odgovor.end();
      }) 
    });
...
streznik.post('/stranka', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    zahteva.session.narocnik = polja.seznamStrank;
    odgovor.redirect('/')
  });
})
